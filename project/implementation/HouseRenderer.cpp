...
class HouseRenderer : public ObjectRenderer {
public:
	...
	void init() override {
		if (TheRenderer::Instance()->renderer->getObjects()->getModel("MD_house_1") == nullptr) {

			ShaderPtr generalShader = TheRenderer::Instance()->renderer->getObjects()->loadShaderFile("general", 0, false, false, false, false, false);
			TheRenderer::Instance()->renderer->getObjects()->loadObjModel("MD_house_1.obj", false, true, generalShader, nullptr);
		}
	}
	...
	void render(std::string cameraName) override {
		...
		TheRenderer::Instance()->renderer->getModelRenderer()->drawModel("MD_house_1", cameraName, modelMatrix, std::vector<std::string>({}));
	}
};

#endif /* defined(HOUSE_RENDERER_H) */
