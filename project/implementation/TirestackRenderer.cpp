#ifndef TIRESTACK_RENDERER_H
#define TIRESTACK_RENDERER_H
...
class TirestackRenderer : public ObjectRenderer {
public:
	...
	void init() override {
		if (TheRenderer::Instance()->renderer->getObjects()->getModel("Tirestack") == nullptr) {

			ShaderPtr generalShader = TheRenderer::Instance()->renderer->getObjects()->loadShaderFile("general", 0, false, false, false, false, false);
			TheRenderer::Instance()->renderer->getObjects()->loadObjModel("Tirestack.obj", false, true, generalShader, nullptr);
		}
	}
	...
	void render(std::string cameraName, ShaderPtr customShader) override {
		...
		TheRenderer::Instance()->renderer->getModelRenderer()->drawModel("Tirestack", cameraName, modelMatrix, std::vector<std::string>({}));
	}
};

#endif /* defined(TIRESTACK_RENDERER_H) */