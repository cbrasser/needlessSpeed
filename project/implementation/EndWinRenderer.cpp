#ifndef END_WIN_RENDERER_CPP
#define END_WIN_RENDERER_CPP
...
class EndWinRenderer : public ObjectRenderer {
public:

    void init() override {

		if (TheRenderer::Instance()->renderer->getObjects()->getModel("quadWin") == nullptr) {
			ShaderPtr endShader = TheRenderer::Instance()->renderer->getObjects()->loadShaderFile("sprite", 0, false, false, false, false, false);

			TheRenderer::Instance()->renderer->getObjects()->loadTexture("winscreen.png");

			TheRenderer::Instance()->renderer->getObjects()->loadObjModel("quadWin.obj", false, true, endShader);
	...
	void render() override {
		...
		TexturePtr texturePtr = TheRenderer::Instance()->renderer->getObjects()->getTexture("winscreen");
		...
		TheRenderer::Instance()->renderer->getModelRenderer()->queueModelInstance(TheRenderer::Instance()->renderer->getObjects()->getModel("quadWin"), "endWin", modelMatrix, viewMatrix, vmml::Matrix4f::IDENTITY, std::vector<std::string>({}));
	}
};

#endif /* defined(END_WIN_RENDERER_CPP) */
