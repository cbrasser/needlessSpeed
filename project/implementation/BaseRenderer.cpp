...
class BaseRenderer : public ObjectRenderer {
public:
	...
	void init() override {
		if (TheRenderer::Instance()->renderer->getObjects()->getModel("Base") == nullptr) {

			ShaderPtr generalShader = TheRenderer::Instance()->renderer->getObjects()->loadShaderFile("general", 0, false, false, false, false, false);
			TheRenderer::Instance()->renderer->getObjects()->loadObjModel("Base.obj", false, true, generalShader, nullptr);
		}
	}
	...
	void render(std::string cameraName) override {
		...
		TheRenderer::Instance()->renderer->getModelRenderer()->drawModel("Base", cameraName, modelMatrix, std::vector<std::string>({}));
	}
};

#endif /* defined(BASE_RENDERER_H) */
