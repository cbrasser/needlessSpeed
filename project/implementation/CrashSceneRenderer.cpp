#ifndef CRASH_SCENE_RENDERER_CPP
#define CRASH_SCENE_RENDERER_CPP
...
class CrashSceneRenderer : public ObjectRenderer {
public:
	void init() override {

		if (TheRenderer::Instance()->renderer->getObjects()->getModel("quadCrash") == nullptr) {
			ShaderPtr menuShader = TheRenderer::Instance()->renderer->getObjects()->loadShaderFile("menu", 0, false, false, false, false, false);

			TheRenderer::Instance()->renderer->getObjects()->loadTexture("CrashScreen.png");

			TheRenderer::Instance()->renderer->getObjects()->loadObjModel("quadCrash.obj", false, true, menuShader);
	...
	void render(std::string cameraName, ShaderPtr customShader) override {
		...
		ShaderPtr crashShader = TheRenderer::Instance()->renderer->getObjects()->getShader("menu");

		TexturePtr texturePtr = TheRenderer::Instance()->renderer->getObjects()->getTexture("CrashScreen");
		crashShader->setUniform("DiffuseMapNew", texturePtr);
		...
		TheRenderer::Instance()->renderer->getModelRenderer()->queueModelInstance(TheRenderer::Instance()->renderer->getObjects()->getModel("quadCrash"), "quadCrash", modelMatrix, viewMatrix, vmml::Matrix4f::IDENTITY, std::vector<std::string>({}));
	}
};

#endif /* defined(CRASH_SCENE_RENDERER_CPP) */