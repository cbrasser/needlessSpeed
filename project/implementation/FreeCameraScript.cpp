	...
	void Update() {
		...
		if (TheRenderer::Instance()->renderer->isRunning()) {
            if(cameraSideward>=1.0f || cameraSideward <= 1.0f){
					...
					TheRenderer::Instance()->renderer->getObjects()->getCamera("camera")->rotateCamera(-1.3f+90*3.1415/180,0.f,0.f);
					...
					TheRenderer::Instance()->renderer->getObjects()->getCamera("camera")->rotateCamera(-90*3.1415/180+1.3f,0.f,0.f);
			}
		}
	}
};
