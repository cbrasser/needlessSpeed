...
{ 
    ...
    intensity = clamp(intensity, 0.05, 0.95); //limit the clamp, so there is not overflow when looking up the colors in the lut (avoids highlighting around the edges)
   
    lowp vec4 modelTexture = texture2D(DiffuseMap, texCoordVarying.st);

    lowp float timeOfDay = clamp(DayNightPulseVarying, 0.05, 0.95);
    lowp float strength = 1.0;
